document.addEventListener("DOMContentLoaded", () => {
    ...
    const faceSort = document.querySelector("#faceSort");
    const faceProducts = document.querySelector("#faceProducts");

    if (faceSort && faceProducts) {
        faceSort.addEventListener("change", () => {
            const productArray = Array.from(faceProducts.children);

            productArray.sort((a, b) => {
                const priceA = Number(a.dataset.price || 0);
                const priceB = Number(b.dataset.price || 0);

                if (faceSort.value === "low") {
                    return priceA - priceB;
                }

                if (faceSort.value === "high") {
                    return priceB - priceA;
                }

                if (faceSort.value === "name") {
                    const nameA =
            ...
            productArray.forEach(product => {
                faceProducts.appendChild(product);
            });
        });
    }
